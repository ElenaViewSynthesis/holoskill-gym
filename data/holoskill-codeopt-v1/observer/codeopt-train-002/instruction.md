...
Increase score_all throughput by batching work, preserving per-item results.

## Context

The repository is checked out at `/app`. The hot path is
`src/batching.py`; the bottleneck family is **async-batching**.

## What you must achieve

1. Keep every test in `tests/` passing. They are authoritative and must
   not be modified.
2. Improve `scoring_throughput_units` as reported by `benchmark.py` (higher is better).
